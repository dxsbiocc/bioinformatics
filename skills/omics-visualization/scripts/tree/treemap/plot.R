...
# Template-ID: tree-treemap
#
# Purpose:
#   Draw a treemap of a hierarchy: rectangle area encodes leaf size.
#
# Inputs:
...
#   Area encodes supplied leaf values. Internal nodes are layout containers;
#   their value is ignored and descendant leaves are summed by the layout.
...
#   value. Path grouping only controls rectangle packing, not a detected
#   community.
...
layout$label_colour <- unname(stats::setNames(label_values, leaf_levels)[as.character(layout$fill_id)])
