...
# Template-ID: graph-circular
#
# Purpose:
#   Draw a circular network from a node table and an edge list.
#
# Inputs:
#   Two tables (tsv/csv):
#     1. nodes: id, name, symbolSize, category
#     2. links: source, target
#
# Output:
#   A PDF, PNG, or SVG circular network plot.
#
# Dependencies:
...
#   Circular placement is a layout choice, not an inferred hierarchy.
#   symbolSize is a supplied display size, not computed here.
...
p <- ggraph::ggraph(graph, layout = "linear", circular = TRUE) +
    ggraph::geom_edge_arc(colour = "#86878c", alpha = 0.25, width = 0.3) +
