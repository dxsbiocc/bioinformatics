...
# Template-ID: radar-basic
#
# Purpose:
#   Draw a radar chart comparing two series across named axes.
#
# Inputs:
#   A wide table with one row per axis. Default example:
#     - name: axis label
#     - remaining columns: one numeric series each (Budget, Spending)
#
# Output:
#   A PDF, PNG, or SVG radar chart.
#
# Dependencies:
...
#   Each row is one axis; remaining columns are comparable series.
...
p <- ggplot(long, aes(
    ...
    geom_polygon(alpha = 0.2, linewidth = 0.8) +
    geom_point(size = 2) +
    geom_text(
        data = axis_lab,
