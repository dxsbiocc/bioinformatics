...
from mcp.opentargets.constants import (
    JSONRPC_VERSION,
...
class OpenTargetsMcpServer:
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-opentargets", "version": "0.1.0"},
        }
...
def serve_stdio(server: OpenTargetsMcpServer | None = None) -> None:
    server = server or OpenTargetsMcpServer()
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"Open Targets MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
