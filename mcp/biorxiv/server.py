...
from mcp.biorxiv.constants import JSONRPC_VERSION, LATEST_PROTOCOL_VERSION, SUPPORTED_PROTOCOL_VERSIONS, JsonObject
...
class BioRxivMcpServer:
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-biorxiv", "version": "0.1.0"},
        }
...
def serve_stdio(server: BioRxivMcpServer | None = None) -> None:
    server = server or BioRxivMcpServer()
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"bioRxiv MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
