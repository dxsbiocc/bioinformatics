"""Small bioRxiv/medRxiv REST API client."""
...
@dataclass
class BioRxivConfig:
    base_url: str = BIORXIV_API_BASE_URL
    ...
    @classmethod
    def from_env(cls) -> "BioRxivConfig":
        return cls(
            base_url=os.environ.get("BIORXIV_API_BASE_URL", BIORXIV_API_BASE_URL),
            contact=os.environ.get("BIORXIV_CONTACT")
            or os.environ.get("MEDRXIV_CONTACT")
            or os.environ.get("NCBI_EMAIL")
            or os.environ.get("ENTREZ_EMAIL"),
            tool=os.environ.get("BIORXIV_TOOL", DEFAULT_TOOL_NAME),
        )
...
class BioRxivClient:
    ...
    def _open_json(self, url: str, label: str) -> tuple[Any, dict[str, str]]:
        ...
        try:
            ...
            raise BioRxivError(f"bioRxiv API {label} returned invalid JSON") from exc
    ...
    def _open_url(self, url: str, label: str, *, accept: str) -> tuple[str, dict[str, str]]:
        ...
        try:
            ...
            raise BioRxivError(f"bioRxiv API {label} returned HTTP {exc.code}: {detail[:500]}") from exc
