#!/usr/bin/env python3
"""Stdio entrypoint for the Bioinformatics PubChem MCP server."""
...
from mcp.pubchem.client import PubChemClient, PubChemConfig
from mcp.pubchem.constants import (
    JSONRPC_VERSION,
    LATEST_PROTOCOL_VERSION,
    SUPPORTED_PROTOCOL_VERSIONS,
    JsonObject,
)
from mcp.pubchem.errors import McpError, PubChemError
from mcp.pubchem.tools import TOOL_HANDLERS, tool_definitions
...
class PubChemMcpServer:
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-pubchem", "version": "0.1.0"},
        }
...
def serve_stdio(server: PubChemMcpServer | None = None) -> None:
    server = server or PubChemMcpServer()
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"PubChem MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
