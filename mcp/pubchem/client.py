...
class PubChemClient:
    ...
    def _open_url(self, url: str, label: str, *, accept: str) -> tuple[str, dict[str, str]]:
        ...
        try:
            ...
            raise PubChemError(f"Could not reach PubChem {label}")
        except httpx.HTTPStatusError as exc:
            detail = exc.response.text
            raise PubChemError(f"PubChem {label} returned HTTP {exc.response.status_code}: {detail[:500]}") from exc
        except httpx.RequestError as exc:
            raise PubChemError(f"Could not reach PubChem {label}: {exc}") from exc
