"""Input validation and provenance helpers for PubChem."""
...
def optional_string(args: JsonObject, name: str, *, default: str | None = None) -> str | None:
...
def optional_positive_identifier(args: JsonObject, name: str) -> str | None:
    value = args.get(name)
    if value is None or value == "":
        return None
    text = str(value).strip()
    if not text.isdigit() or int(text) <= 0:
        raise McpError(-32602, f"{name} must be a positive integer identifier")
    return text
