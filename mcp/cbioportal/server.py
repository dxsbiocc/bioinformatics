...
from mcp.cbioportal.constants import JSONRPC_VERSION, LATEST_PROTOCOL_VERSION, SUPPORTED_PROTOCOL_VERSIONS, JsonObject
...
class CbioPortalMcpServer:
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-cbioportal", "version": "0.1.0"},
        }
...
def serve_stdio(server: CbioPortalMcpServer | None = None) -> None:
    server = server or CbioPortalMcpServer()
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"cBioPortal MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
