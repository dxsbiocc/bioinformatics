...
from .constants import CHEMBL_REST_BASE_URL, CHEMBL_WEBSITE_BASE_URL, DEFAULT_TOOL_NAME, JsonObject
from .errors import ChemblError


@dataclass
class ChemblConfig:
    base_url: str = CHEMBL_REST_BASE_URL
    ...
    @classmethod
    def from_env(cls) -> ChemblConfig:
        return cls(
            base_url=os.environ.get("CHEMBL_REST_BASE_URL", CHEMBL_REST_BASE_URL),
            website_base_url=os.environ.get("CHEMBL_WEBSITE_BASE_URL", CHEMBL_WEBSITE_BASE_URL),
            contact=os.environ.get("CHEMBL_CONTACT")
            or os.environ.get("UNIPROT_CONTACT")
            or os.environ.get("NCBI_EMAIL")
            or os.environ.get("ENTREZ_EMAIL"),
            tool=os.environ.get("CHEMBL_TOOL", DEFAULT_TOOL_NAME),
        )
...
class ChemblClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.config = config or ChemblConfig.from_env()
        self._opener = opener
        self._sleep = sleep
    ...
    def _open_json(self, url: str, label: str) -> tuple[Any, dict[str, str]]:
        ...
        try:
            ...
            raise ChemblError(f"ChEMBL {label} returned invalid JSON") from exc
    ...
    def _open_url(self, url: str, label: str, *, accept: str) -> tuple[str, dict[str, str]]:
        ...
        try:
            ...
            raise ChemblError(f"Could not reach ChEMBL {label}")
        except httpx.HTTPStatusError as exc:
            detail = exc.response.text
            raise ChemblError(f"ChEMBL {label} returned HTTP {exc.response.status_code}: {detail[:500]}") from exc
        except httpx.RequestError as exc:
            raise ChemblError(f"Could not reach ChEMBL {label}: {exc}") from exc
