...
from mcp.chembl.constants import (
    JSONRPC_VERSION,
...
class ChemblMcpServer:
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-chembl", "version": "0.1.0"},
        }
...
def serve_stdio(server: ChemblMcpServer | None = None) -> None:
    server = server or ChemblMcpServer()
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"ChEMBL MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
