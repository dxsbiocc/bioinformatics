"""Small BioStudies REST API client."""
...
from .constants import BIOSTUDIES_API_BASE_URL, BIOSTUDIES_WEBSITE_BASE_URL, DEFAULT_TOOL_NAME, JsonObject
from .errors import BioStudiesError


@dataclass
class BioStudiesConfig:
    base_url: str = BIOSTUDIES_API_BASE_URL
    ...
    @classmethod
    def from_env(cls) -> BioStudiesConfig:
        return cls(
            base_url=os.environ.get("BIOSTUDIES_API_BASE_URL", BIOSTUDIES_API_BASE_URL),
            website_base_url=os.environ.get("BIOSTUDIES_WEBSITE_BASE_URL", BIOSTUDIES_WEBSITE_BASE_URL),
            contact=os.environ.get("BIOSTUDIES_CONTACT")
            or os.environ.get("UNIPROT_CONTACT")
            or os.environ.get("NCBI_EMAIL")
            or os.environ.get("ENTREZ_EMAIL"),
            tool=os.environ.get("BIOSTUDIES_TOOL", DEFAULT_TOOL_NAME),
        )
...
class BioStudiesClient:
    ...
    def __init__(
        self,
        config: BioStudiesConfig | None = None,
        *,
        opener: Callable[[httpx.Request, float], str] | None = None,
        sleep: Callable[[float], None] = time.sleep,
        monotonic: Callable[[], float] = time.monotonic,
    ) -> None:
        self.config = config or BioStudiesConfig.from_env()
        self._opener = opener
        self._sleep = sleep
    ...
    def _open_json(self, url: str, label: str) -> tuple[Any, dict[str, str]]:
        ...
        try:
            ...
            raise BioStudiesError(f"BioStudies {label} returned invalid JSON") from exc
    ...
    def _open_url(self, url: str, label: str, *, accept: str) -> tuple[str, dict[str, str]]:
        ...
        try:
            ...
            raise BioStudiesError(f"Could not reach BioStudies {label}")
        except httpx.HTTPStatusError as exc:
            detail = exc.response.text
            raise BioStudiesError(f"BioStudies {label} returned HTTP {exc.response.status_code}: {detail[:500]}") from exc
        except httpx.RequestError as exc:
            raise BioStudiesError(f"Could not reach BioStudies {label}: {exc}") from exc
