...
from mcp.chebi.constants import JSONRPC_VERSION, LATEST_PROTOCOL_VERSION, SUPPORTED_PROTOCOL_VERSIONS, JsonObject
from mcp.chebi.errors import ChebiError, McpError
from mcp.chebi.tools import TOOL_HANDLERS, tool_definitions


class ChebiMcpServer:
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-chebi", "version": "0.1.0"},
        }
...
def serve_stdio(server: ChebiMcpServer | None = None) -> None:
    server = server or ChebiMcpServer()
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"ChEBI MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
