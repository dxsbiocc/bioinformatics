...
from mcp.efo.constants import JSONRPC_VERSION, LATEST_PROTOCOL_VERSION, SUPPORTED_PROTOCOL_VERSIONS, JsonObject
...
class EfoMcpServer:
    def __init__(self, client: EfoClient | None = None) -> None:
        self.client = client or EfoClient()
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-efo", "version": "0.1.0"},
        }
...
def serve_stdio(server: EfoMcpServer | None = None) -> None:
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"EFO MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
