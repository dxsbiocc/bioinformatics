#!/usr/bin/env python3
"""Stdio entrypoint for the Bioinformatics gnomAD MCP server."""
...
from mcp.gnomad.client import GnomadClient, GnomadConfig
from mcp.gnomad.constants import (
    JSONRPC_VERSION,
    LATEST_PROTOCOL_VERSION,
    SUPPORTED_PROTOCOL_VERSIONS,
    JsonObject,
)
from mcp.gnomad.errors import GnomadError, McpError
from mcp.gnomad.tools import TOOL_HANDLERS, tool_definitions


class GnomadMcpServer:
    def __init__(self, client: GnomadClient | None = None) -> None:
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-gnomad", "version": "0.1.0"},
        }
...
def serve_stdio(server: GnomadMcpServer | None = None) -> None:
    server = server or GnomadMcpServer()
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"gnomAD MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
