...
from mcp.quickgo.constants import JSONRPC_VERSION, LATEST_PROTOCOL_VERSION, SUPPORTED_PROTOCOL_VERSIONS, JsonObject
from mcp.quickgo.errors import McpError, QuickGoError
from mcp.quickgo.tools import TOOL_HANDLERS, tool_definitions


class QuickGoMcpServer:
    def __init__(self, client: QuickGoClient | None = None) -> None:
        self.client = client or QuickGoClient()
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-quickgo", "version": "0.1.0"},
        }
...
def serve_stdio(server: QuickGoMcpServer | None = None) -> None:
    server = server or QuickGoMcpServer()
