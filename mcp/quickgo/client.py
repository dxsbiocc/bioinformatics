...
from .constants import DEFAULT_TOOL_NAME, QUICKGO_API_BASE_URL, QUICKGO_WEBSITE_BASE_URL, JsonObject
from .errors import QuickGoError


@dataclass
class QuickGoConfig:
    base_url: str = QUICKGO_API_BASE_URL
    ...
    def from_env(cls) -> QuickGoConfig:
        return cls(
            ...
            contact=os.environ.get("QUICKGO_CONTACT")
            or os.environ.get("UNIPROT_CONTACT")
            or os.environ.get("NCBI_EMAIL")
            or os.environ.get("ENTREZ_EMAIL"),
            tool=os.environ.get("QUICKGO_TOOL", DEFAULT_TOOL_NAME),
        )
...
class QuickGoClient:
    ...
    def __init__(
        self,
        config: QuickGoConfig | None = None,
        *,
        opener: Callable[[httpx.Request, float], str] | None = None,
        sleep: Callable[[float], None] = time.sleep,
        monotonic: Callable[[], float] = time.monotonic,
    ) -> None:
        self.config = config or QuickGoConfig.from_env()
        self._opener = opener
        self._sleep = sleep
    ...
    def _open_json(self, url: str, label: str) -> tuple[Any, dict[str, str]]:
        ...
        try:
            ...
            raise QuickGoError(f"QuickGO {label} returned invalid JSON") from exc
    ...
    def _open_url(self, url: str, label: str, *, accept: str) -> tuple[str, dict[str, str]]:
        ...
        try:
            ...
            raise QuickGoError(f"Could not reach QuickGO {label}")
        except httpx.HTTPStatusError as exc:
            detail = exc.response.text
            raise QuickGoError(f"QuickGO {label} returned HTTP {exc.response.status_code}: {detail[:500]}") from exc
        except httpx.RequestError as exc:
            raise QuickGoError(f"Could not reach QuickGO {label}: {exc}") from exc
