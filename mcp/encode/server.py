...
from mcp.encode.constants import JSONRPC_VERSION, LATEST_PROTOCOL_VERSION, SUPPORTED_PROTOCOL_VERSIONS, JsonObject
from mcp.encode.errors import EncodeError, McpError
from mcp.encode.tools import TOOL_HANDLERS, tool_definitions


class EncodeMcpServer:
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-encode", "version": "0.1.0"},
        }
...
def serve_stdio(server: EncodeMcpServer | None = None) -> None:
    server = server or EncodeMcpServer()
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"ENCODE MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
