...
from mcp.gwas.constants import (
    JSONRPC_VERSION,
...
class GwasMcpServer:
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-gwas", "version": "0.1.0"},
        }
...
def serve_stdio(server: GwasMcpServer | None = None) -> None:
    server = server or GwasMcpServer()
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"GWAS Catalog MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
