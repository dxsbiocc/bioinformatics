"""Small GWAS Catalog REST API v2 client."""
...
from .constants import DEFAULT_TOOL_NAME, GWAS_REST_BASE_URL, GWAS_WEBSITE_BASE_URL, JsonObject
from .errors import GwasError


@dataclass
class GwasConfig:
    base_url: str = GWAS_REST_BASE_URL
    ...
    def from_env(cls) -> "GwasConfig":
        return cls(
            ...
            contact=os.environ.get("GWAS_CONTACT")
            or os.environ.get("UNIPROT_CONTACT")
            or os.environ.get("NCBI_EMAIL")
            or os.environ.get("ENTREZ_EMAIL"),
            tool=os.environ.get("GWAS_TOOL", DEFAULT_TOOL_NAME),
        )
...
class GwasClient:
    ...
    def __init__(
        self,
        config: GwasConfig | None = None,
        *,
        opener: Callable[[httpx.Request, float], str] | None = None,
        sleep: Callable[[float], None] = time.sleep,
        monotonic: Callable[[], float] = time.monotonic,
    ) -> None:
        self.config = config or GwasConfig.from_env()
        self._opener = opener
        self._sleep = sleep
    ...
    def _open_json(self, url: str, label: str) -> tuple[Any, dict[str, str]]:
        ...
        try:
            ...
            raise GwasError(f"GWAS Catalog {label} returned invalid JSON") from exc
    ...
    def _open_url(self, url: str, label: str, *, accept: str) -> tuple[str, dict[str, str]]:
        ...
        try:
            ...
            raise GwasError(f"Could not reach GWAS Catalog {label}")
        except httpx.HTTPStatusError as exc:
            detail = exc.response.text
            raise GwasError(
                f"GWAS Catalog {label} returned HTTP {exc.response.status_code}: {detail[:500]}"
            ) from exc
        except httpx.RequestError as exc:
            raise GwasError(f"Could not reach GWAS Catalog {label}: {exc}") from exc
