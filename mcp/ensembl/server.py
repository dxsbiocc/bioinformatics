...
from mcp.ensembl.constants import (
    JSONRPC_VERSION,
...
class EnsemblMcpServer:
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-ensembl", "version": "0.1.0"},
        }
...
def serve_stdio(server: EnsemblMcpServer | None = None) -> None:
    server = server or EnsemblMcpServer()
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"Ensembl MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
