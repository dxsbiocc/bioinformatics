    ...
    def _open_url(
        ...
        *,
        ...
        try:
            ...
            raise EnsemblError(f"Could not reach Ensembl {label}")
        except httpx.HTTPStatusError as exc:
            detail = exc.response.text
            raise EnsemblError(
                f"Ensembl {label} returned HTTP {exc.response.status_code}: {detail[:500]}"
            ) from exc
        except httpx.RequestError as exc:
            raise EnsemblError(f"Could not reach Ensembl {label}: {exc}") from exc
