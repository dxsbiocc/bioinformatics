...
from mcp.hmdb.constants import JSONRPC_VERSION, LATEST_PROTOCOL_VERSION, SUPPORTED_PROTOCOL_VERSIONS, JsonObject
...
class HmdbMcpServer:
    def __init__(self, client: HmdbClient | None = None) -> None:
        self.client = client or HmdbClient()
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-hmdb", "version": "0.1.0"},
        }
...
def serve_stdio(server: HmdbMcpServer | None = None) -> None:
    server = server or HmdbMcpServer()
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"HMDB MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
