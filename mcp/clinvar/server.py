...
from mcp.clinvar.constants import (
    JSONRPC_VERSION,
...
class ClinvarMcpServer:
    def __init__(self, client: ClinvarClient | None = None) -> None:
        self.client = client or ClinvarClient()
    ...
    def _initialize(self, params: JsonObject) -> JsonObject:
        ...
        return {
            ...
            "serverInfo": {"name": "bioinformatics-clinvar", "version": "0.1.0"},
        }
...
def serve_stdio(server: ClinvarMcpServer | None = None) -> None:
    server = server or ClinvarMcpServer()
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"ClinVar MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
