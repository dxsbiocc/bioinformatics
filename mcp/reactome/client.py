"""Small Reactome ContentService API client."""
...
class ReactomeConfig:
    base_url: str = REACTOME_CONTENT_API_BASE_URL
    website_base_url: str = REACTOME_WEBSITE_BASE_URL
    ...
    def from_env(cls) -> ReactomeConfig:
        return cls(
            ...
            contact=os.environ.get("REACTOME_CONTACT")
            or os.environ.get("UNIPROT_CONTACT")
            or os.environ.get("NCBI_EMAIL")
            or os.environ.get("ENTREZ_EMAIL"),
            tool=os.environ.get("REACTOME_TOOL", DEFAULT_TOOL_NAME),
        )
...
class ReactomeClient:
    ...
    def __init__(
        self,
        config: ReactomeConfig | None = None,
        *,
        opener: Callable[[httpx.Request, float], str] | None = None,
        sleep: Callable[[float], None] = time.sleep,
        monotonic: Callable[[], float] = time.monotonic,
    ) -> None:
        self.config = config or ReactomeConfig.from_env()
        self._opener = opener
        self._sleep = sleep
    ...
    def _open_json(self, url: str, label: str) -> tuple[Any, dict[str, str]]:
        ...
        try:
            ...
            raise ReactomeError(f"Reactome {label} returned invalid JSON") from exc
    ...
    def _open_url(
        ...
        *,
        ...
        try:
            ...
            raise ReactomeError(f"Could not reach Reactome {label}")
        except httpx.HTTPStatusError as exc:
            detail = exc.response.text
            raise ReactomeError(
                f"Reactome {label} returned HTTP {exc.response.status_code}: {detail[:500]}"
            ) from exc
        except httpx.RequestError as exc:
            raise ReactomeError(f"Could not reach Reactome {label}: {exc}") from exc
