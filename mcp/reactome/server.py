...
from mcp.reactome.constants import (
    JSONRPC_VERSION,
...
def serve_stdio(server: ReactomeMcpServer | None = None) -> None:
    server = server or ReactomeMcpServer()
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"Reactome MCP internal error: {exc}", file=sys.stderr)
            response = internal_error_response(request_id, exc)
