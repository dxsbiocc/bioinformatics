"""Small REST client for MetaboLights and EBI Search."""
...
class MetaboLightsConfig:
    ...
    def from_env(cls) -> MetaboLightsConfig:
        return cls(
            ...
            website_base_url=os.environ.get("METABOLIGHTS_WEBSITE_BASE_URL", METABOLIGHTS_WEBSITE_BASE_URL),
            contact=os.environ.get("METABOLIGHTS_CONTACT")
            or os.environ.get("EBI_CONTACT")
            or os.environ.get("UNIPROT_CONTACT")
            or os.environ.get("NCBI_EMAIL"),
            tool=os.environ.get("METABOLIGHTS_TOOL", DEFAULT_TOOL_NAME),
        )
...
class MetaboLightsClient:
    ...
    def __init__(
        self,
        config: MetaboLightsConfig | None = None,
        *,
        opener: Callable[[httpx.Request, float], str] | None = None,
        sleep: Callable[[float], None] = time.sleep,
        monotonic: Callable[[], float] = time.monotonic,
    ) -> None:
        self.config = config or MetaboLightsConfig.from_env()
        self._opener = opener
        self._sleep = sleep
    ...
    def request_ws_json_with_headers(
        self,
        endpoint: str,
        params: JsonObject | None = None,
    ) -> tuple[Any, dict[str, str]]:
        return self._open_json(self._build_url(self.config.ws_base_url, endpoint, params or {}), endpoint)
    ...
    def request_search_json_with_headers(
        ...
        return self._open_json(self._build_url(self.config.search_base_url, endpoint, params or {}), endpoint)

    def build_ws_url(self, endpoint: str, params: JsonObject | None = None) -> str:
        return self._build_url(self.config.ws_base_url, endpoint, params or {})

    def build_search_url(self, endpoint: str, params: JsonObject | None = None) -> str:
        return self._build_url(self.config.search_base_url, endpoint, params or {})
    ...
    def _open_json(self, url: str, label: str) -> tuple[Any, dict[str, str]]:
        ...
        try:
            ...
            raise MetaboLightsError(f"MetaboLights {label} returned invalid JSON") from exc
    ...
    def _open_url(self, url: str, label: str, *, accept: str) -> tuple[str, dict[str, str]]:
        ...
        try:
            ...
            raise MetaboLightsError(f"Could not reach MetaboLights {label}")
        except httpx.HTTPStatusError as exc:
            detail = exc.response.text
            raise MetaboLightsError(f"MetaboLights {label} returned HTTP {exc.response.status_code}: {detail[:500]}") from exc
        except httpx.RequestError as exc:
            raise MetaboLightsError(f"Could not reach MetaboLights {label}: {exc}") from exc
