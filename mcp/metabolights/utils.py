"""Input validation and provenance helpers for MetaboLights."""
...
def require_accession(args: JsonObject, name: str = "accession") -> str:
    ...
    if not MTBLS_RE.match(value):
        raise McpError(-32602, f"{name} must look like MTBLS1")
    return value
...
def strip_html(value: Any) -> str:
    ...
    if not text:
        return ""
    return normalize_space(unescape(HTML_TAG_RE.sub(" ", text)))
